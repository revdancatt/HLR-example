...
// Standard A3 paper size is 420mm x 297mm, this gives us that ratio
const ratio = 420 / 297
...
const layoutCanvas = async () => {
  //  Kill the next animation frame, just incase
  window.cancelAnimationFrame(nextFrame)

  // Work out all the widths and heights for display
  ...
  // Do something special for high res mode
  if (highRes) {
    canvas.height = 8192
...
// This downloads a png version
const autoDownloadCanvas = async (showHash = false) => {
  const element = document.createElement('a')
  element.setAttribute('download', `HLR_${fxhash}`)
  element.style.display = 'none'
  document.body.appendChild(element)
  let imageBlob = null
